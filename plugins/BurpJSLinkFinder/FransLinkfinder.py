...
class BurpExtender(IBurpExtender, IScannerCheck, ITab):
    ...
    def initUI(self):
        ...
        layoutf.setHorizontalGroup(
            layoutf.createParallelGroup().addGroup(
                layoutf.createSequentialGroup().addGroup(
        ...
        layoutm.setHorizontalGroup(
            layoutm.createParallelGroup().addGroup(
                layoutm.createSequentialGroup().addGroup(
            ...
            if ".js" in str(urlReq):
                ...
                if any(x in testString for x in JSExclusionList):
                    ...
                    for counter, issueText in enumerate(issueText):
                        self.outputTxtArea.append("\n" + "\t" + issueText["link"])
                        if linkA.valcheckFullURL(
                            issueText["link"]
                        ) and linkA.valcheckMappedList(
                            issueText["link"], self.mapTxtArea
                        ):
                            self.mapTxtArea.append("\n" + issueText["link"])
                            full_urls += [issueText["link"]]
                        elif not linkA.valcheckFullURL(issueText["link"]):
                            fullURL = urlparse.urljoin(
                                urlparse.urljoin(str(urlReq), "/"), issueText["link"]
                            )
                        ...
                        if linkA.isNotBlank((filNam)):
                            ...
                            if (linkA.checkValidFile(filNam)) and (
                                filNam not in self.filesTxtArea.text
                            ):
    ...
    def URL_SPLITTER(self, url):
        ...
        try:
            URL_HOST_SERVICE = self.helpers.buildHttpService(
                URL_HOSTNAME, URL_PORT, URL_PROTOCAL
            )
        except java.lang.IllegalArgumentException:
            self.callbacks.printOutput(
                "EXCEPTION BECAUSE HTTPSERVICE VALUES IS INVALID : {} : ".format(url)
            )
            self.callbacks.printOutput(
                "EXCEPTION VALUES ARE :", URL_HOSTNAME, URL_PORT, URL_PROTOCAL
            )
        return (
            URL_SPLIT,
            URL_PROTOCAL,
...
class linkAnalyse:
    ...
    def parser_file(self, content, regex_str, mode=1, more_regex=None, no_dup=1):
        ...
        items = [
            {"link": m.group(1), "start": m.start(1), "end": m.end(1)}
...
class SRI(IScanIssue, ITab):
    ...
    def getSeverity(self):
        return (
            "Information"  # "High", "Medium", "Low", "Information" or "False positive"
        )
    ...
    def getIssueBackground(self):
        return str(
            "JS files holds links to other parts of web applications. Refer to TAB for results."
        )
